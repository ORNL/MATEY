...
class SpaceTimeBlock(nn.Module):
    ...
    def forward(self, x, bcs, leadtime=None, t_pos_area=None):
        # input is t x b x c x h x w 
        # t_pos_area: [b, t, h, w, 4]
        T, B, C, H, W = x.shape

        # Time attention
        x = self.temporal(x, leadtime=leadtime, t_pos_area=t_pos_area) # Residual in block
        ...
        if t_pos_area is not None:
            t_pos_area = rearrange(t_pos_area, 'b t h w  c-> (t b) h w c')
        
        if hasattr(self, 'spatial'):
            x = self.spatial(x, bcs, t_pos_area=t_pos_area) # Convnext has the residual in the block
        else:
...
class SpaceTimeBlock_svit(nn.Module):
    ...
    def forward(self, x, bcs, leadtime=None, mask_padding=None, t_pos_area=None):
        ...
        if mask_padding is not None:
            ...
            x[mask_padding] = self.temporal(x[mask_padding], leadtime=leadtime[mask_padding] 
                                            if leadtime is not None else leadtime)#, t_pos_area=t_pos_area_time) # Residual in block
            # Now do spatial attention
            ...
            x = self.spatial(x, bcs=None, mask_padding=mask_padding) #, t_pos_area=t_pos_area_xy) # Convnext has the residual in the block            
        else:
            #t_pos_area_time = rearrange(t_pos_area, 'b t slen c -> (b slen t) c')
            x = self.temporal(x, leadtime=leadtime)#, t_pos_area=t_pos_area_time) # Residual in block
            # Now do spatial attention
            x = rearrange(x, '(b slen) c t -> (b t) c slen', slen=slen)
            #t_pos_area_xy = rearrange(t_pos_area, 'b t slen c -> (b t slen) c')
            x = self.spatial(x, bcs=None)#, t_pos_area=t_pos_area_xy) # Convnext has the residual in the block
...
class SpaceTimeBlock_all2all(nn.Module):
    ...
    def forward(self, x, bcs=None, leadtime=None, mask_padding=None, t_pos_area=None):
        # x:  b x c x tslen
        # leadtime: [b, c]
        # mask_padding: [b, slen]
        # t_pos_area:[b*tslen, 4]
        B, C, tslen = x.shape
        if mask_padding is not None:
        ...
        # attention
        x = self.spatialtemporal(x, leadtime=leadtime, mask_padding=mask_padding, t_pos_area=t_pos_area) # Residual in block
        return x
