...
def build_vit(params):
    ...
    """
    model = ViT_all2all(patch_size=params.patch_size,
...
class ViT_all2all(nn.Module):
    ...
    Args:
        ...
        sts_f
    """
    def __init__(self, patch_size=(16, 16), embed_dim=768,  num_heads=12, processor_blocks=8, n_states=6,
    ...
    def get_patchsequence(self, x,  state_labels, refineind=None, leadtime=None):
        ...
        #       x_local: [npatches, C_emb, T*ps0/pso_ref*ps1/ps1_ref] 
        #       leadtime_local: [npatches, 1]
        #       t_pos_area: [B,T, ntoken_x*ntoken_y, 4] 
        #       t_pos_area_local: [npatches, T, nt_ref_x*n_ref_y, 4]
        #     else: return tensors
        #       x_padding: [B, C_emb, T*ntoken_len_tot]
        #       patch_ids_ref: [npatches] (ids of effective tokens in x_local)
        #       patch_ids: [npatches] #selected token ids with sample pos inside batch considered
        #       t_pos_area: [B, T, ntoken_len_tot, 4]
        """
        ...
        if refineind is None:
            x = rearrange(x, 't b c hw -> b c (t hw)')
        ...
        if self.sts_model:
            x = rearrange(x, 't b c hw -> b c (t hw)')
            x_local = rearrange(x_local, 't nrfb c hw -> nrfb c (t hw)')
        ...
        #x_padding: [B, C_emb, T*ntoken_tot]
        #mask_padding: [B, ntoken_tot]
        return x_padding, patch_ids, patch_ids_ref, mask_padding, None, None, t_pos_area_padding, None
    
    def get_spatiotemporalfromsequence(self, x_padding, patch_ids, patch_ids_ref, state_labels, T, H, W):
        #taking token sequences, x_padding, in shape [B, C_emb, T*ntoken_tot] as input
        #return [T, B, C, H, W]
        B, _, _ = x_padding.shape
    ...
    def add_sts_model(self, xbase, patch_ids, x_local, state_labels, bcs, leadtime=None, t_pos_area=None):
        ...
        # Decode -
        x_local = rearrange(x_local, 'nrfb c (t h w) -> (t nrfb) c h w', t=T, h=ntx_ref)
    ...
    def forward(self, x, state_labels, bcs, leadtime=None, refineind=None, returnbase4train=False):
        ...
        for iblk, blk in enumerate(self.blocks):
            if iblk==0:
                x_padding = blk(x_padding, bcs, leadtime=leadtime, mask_padding=mask_padding)#, t_pos_area=tposarea_padding)
            else:
                x_padding = blk(x_padding, bcs, leadtime=None, mask_padding=mask_padding)
        self.debug_nan(x_padding, message="attention block")
        ################################################################################
        ######## Decode ########
        if self.sts_model:
            xbase = self.get_spatiotemporalfromsequence(x_padding, None, None, state_labels, T, H, W)
            x = self.add_sts_model(xbase, patch_ids, x_local, state_labels, bcs, leadtime=leadtime_local, t_pos_area=tposarea_local)
        else:
            x = self.get_spatiotemporalfromsequence(x_padding, patch_ids, patch_ids_ref, state_labels, T, H, W)
        ######### Denormalize ########
