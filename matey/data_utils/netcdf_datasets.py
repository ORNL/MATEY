...
class BasenetCDFDirectoryDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        time_idx += nsteps
        # FIXME:this should depend on autoregressive or not, not on the input_control_act flag
        if leadtime is None and self.input_control_act==False:
        ...
        try:
            if self.input_control_act==True:
                ...
                trajectory, leadtime = self._reconstruct_sample(self.datasets[file_idx], leadtime, self.input_control_act, time_idx, nsteps)
        ...
        if len(patch_size)==2 and (self.refine_ratio is not None or self.gammaref is not None):
            refineind = get_top_variance_patchids(patch_size, trajectory[0:self.n_steps], self.gammaref, self.refine_ratio)

            return trajectory[0:self.n_steps], torch.as_tensor(bcs), traj_out.astype(np.float32), refineind, leadtime, input_control.astype(np.float32)
        if self.input_control_act==True:
            return trajectory[0:self.n_steps], torch.as_tensor(bcs), traj_out.astype(np.float32), leadtime, input_control.astype(np.float32)
        else:
            return trajectory[0:self.n_steps], torch.as_tensor(bcs), traj_out.astype(np.float32), leadtime
