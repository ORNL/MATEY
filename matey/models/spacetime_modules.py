...
class SpaceTimeBlock(nn.Module):
    ...
    def forward(self, x, bcs, sequence_parallel_group=None, leadtime=None, input_control=None, t_pos_area=None):
        # input is: [T, B, C, D, H, W ]
        # t_pos_area: [b, t, d, h, w, 5]
        T = x.shape[0]

        # Time attention
        x = self.temporal(x, leadtime=leadtime, input_control=input_control, t_pos_area=t_pos_area, sequence_parallel_group=sequence_parallel_group) # Residual in block
...
class SpaceTimeBlock_svit(nn.Module):
    ...
    def forward(self, x, bcs, sequence_parallel_group=None, leadtime=None, input_control=None, mask_padding=None, t_pos_area=None):
        ...
        if input_control is not None:
            input_control = input_control.repeat_interleave(slen, dim=0)
        if mask_padding is not None:
            ...
            x[mask_padding] = self.temporal(x[mask_padding], leadtime=leadtime[mask_padding]
                                            if leadtime is not None else leadtime, input_control=input_control[mask_padding]
                                            if input_control is not None else input_control, sequence_parallel_group=sequence_parallel_group)#, t_pos_area=t_pos_area_time) # Residual in block
            # Now do spatial attention
            ...
            x = self.temporal(x, leadtime=leadtime, input_control=input_control, sequence_parallel_group=sequence_parallel_group)#, t_pos_area=t_pos_area_time) # Residual in block
            # Now do spatial attention
            x = rearrange(x, '(b slen) c t -> (b t) c slen', slen=slen)
            #t_pos_area_xy = rearrange(t_pos_area, 'b t slen c -> (b t slen) c')
