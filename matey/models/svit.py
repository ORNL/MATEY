...
class sViT_all2all(BaseModel):
    ...
    def __init__(self, tokenizer_heads=None, embed_dim=768, space_type="all2all", time_type="all2all", num_heads=12, processor_blocks=8, n_states=6, n_states_cond=None,
                 drop_path=.2, sts_train=False, sts_model=False, leadtime=False, autoregressive=False, input_control=False, n_steps=1, bias_type="none", replace_patch=True, SR_ratio=[1,1,1], hierarchical=None):
    ...
    def forward(self, x, state_labels, bcs, sequence_parallel_group=None, leadtime=None, input_control=None, returnbase4train=False, 
        ...
        for iblk, blk in enumerate(self.blocks):
            ...
            if iblk==0:
                x_padding = blk(x_padding, bcs, sequence_parallel_group=sequence_parallel_group, leadtime=leadtime, input_control=input_control, mask_padding=mask_padding)
            else:
                x_padding = blk(x_padding, bcs, sequence_parallel_group=sequence_parallel_group, leadtime=None, input_control=None, mask_padding=mask_padding)
