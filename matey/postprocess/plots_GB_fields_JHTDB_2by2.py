...
caselabels=["ViT-PS64", "ViT-PS32", "Matey-PS8", "Matey-Small", "Matey-Medium"]
...
for icase, (caselabel, casestr, output_dir) in enumerate(zip(caselabels, casestrs, casedirs)):
    ...
    if iplot==2:
        im=axs[0,0].contourf(tar_total[0,0,0,:,:], cmap="jet", levels=50)
        axs[0,0].set_aspect('equal')
        axs[0,0].axis('off')
        axs[0,0].set_title("True Vx")
        divider = make_axes_locatable(axs[0,0])
        cax = divider.append_axes('right', size='5%', pad=0.05)
        plt.colorbar(im, cax=cax, orientation='vertical')
    irow=(iplot+1)//2
    icol=(iplot+1)%2
    im=axs[irow,icol].contourf(pred_total[0,0,0,:,:], cmap="jet", levels=50)
...
plt.subplots_adjust(left=0.08, bottom=0.08, right=0.92, top=0.90, wspace=0.2, hspace=0.2)
plt.savefig("/lustre/orion/lrn037/scratch/zhangp/fy25/GB/matey/GB/JHTDB_Ufield.png")
plt.savefig("/lustre/orion/lrn037/scratch/zhangp/fy25/GB/matey/GB/JHTDB_Ufield.pdf")
