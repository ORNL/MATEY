...
for icase, (caselabel, cutoff) in enumerate(zip(caselabels, cutoffs)):
    ...
    pdf, f1_centers= turbulence_des.compute_pdf(np.log10(OMGpre/OMGpre.mean()), bins=200)
    axs[0].plot(f1_centers, pdf, lines[iplot], color=colors[iplot], label=caselabel, linewidth=2.0)
    axs[0].set_title("Enstrophy")
    pdf, f1_centers= turbulence_des.compute_pdf(np.log10(EPSIpre/EPSIpre.mean()), bins=200)
...
plt.savefig("/lustre/orion/lrn037/scratch/zhangp/fy25/GB/matey/GB/pdfs_omg_epsi.png")
plt.savefig("/lustre/orion/lrn037/scratch/zhangp/fy25/GB/matey/GB/pdfs_omg_epsi.pdf")
###############################################################################################
###############################################################################################
