import torch
import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1 import make_axes_locatable
import sys, math
from einops import rearrange


def normalize_spatiotemporal_persample(x):
    # input tensor shape: [T, B, C, H, W]
    ######## Normalize (time + space per sample)########
    with torch.no_grad():
        data_std, data_mean = torch.std_mean(x, dim=(0, -2, -1), keepdims=True)
        data_std = data_std + 1e-7 # Orig 1e-7
    x = (x - data_mean) / (data_std)
    return x, data_mean, data_std

def get_top_variance_patchids(patch_size, data, gammaref, refine_ratio):
        #FIXME: need to figure out better way to extend to more levels
        assert len(patch_size)<3, "Implemented for two levels for now"
        T, nvar, H, W = data.shape
        ps = patch_size[-1]
        ps0=ps[0]
        ps1=ps[1]
        ntokenx = H//ps0
        ntokeny = W//ps1
        variance = torch.empty(ntokenx*ntokeny)
        xdata = torch.from_numpy(np.asarray(data))
        II_topk = torch.empty(ntokenx*ntokeny, dtype=torch.int).fill_(-1)
        for ix in range(ntokenx):
            for iy in range(ntokeny):
                variance[ix*ntokeny+iy]=xdata[:, :, ix*ps0:(ix+1)*ps0, iy*ps1:(iy+1)*ps1].var(dim=(0,-2,-1)).mean()
        if gammaref is None:
            nrefines = int(refine_ratio*ntokenx*ntokeny)
            _, II_t = variance.topk(nrefines)
        else:
            varmax = variance.max()
            b = variance > varmax*gammaref
            II_t = b.nonzero().type(torch.int)[:,0]
        II_topk[:len(II_t)]=II_t
        return II_topk

def plot_checking(patch_size, field_names, time_idx, data):
    T, nvar, H, W = data.shape
    ps = patch_size[-1]
    ps0=ps[0]
    ps1=ps[1]
    ntokenx = H//ps0
    ntokeny = W//ps1
    ncol=math.ceil(math.sqrt(nvar))
    nrow=nvar//ncol

    fig, axs = plt.subplots(nrow,ncol,figsize=(20, 20))
    for ivar in range(nvar):
        irow=ivar//ncol
        icol=ivar%ncol
        ax = axs[irow, icol]
        cs = ax.contourf(np.asarray(data[0, ivar, :-20, :]).squeeze(), cmap="jet", levels=50)
        for iref in range(ntokenx*ntokeny):
            itoken = iref
            ix=itoken//ntokeny
            iy=itoken%ntokeny
            assert ix*ntokeny+iy==itoken
            ax.plot([iy*ps1,     (iy+1)*ps1], [ix*ps0,         ix*ps0], "m--")
            ax.plot([iy*ps1,     (iy+1)*ps1], [(ix+1)*ps0, (ix+1)*ps0], "m--")
            ax.plot([iy*ps1,         iy*ps1], [ix*ps0,     (ix+1)*ps0], "m--")
            ax.plot([(iy+1)*ps1, (iy+1)*ps1], [ix*ps0,     (ix+1)*ps0], "m--")
            variance=data[:, ivar, ix*ps0:(ix+1)*ps0, iy*ps1:(iy+1)*ps1].var(axis=(0,-2,-1)).mean()
            ax.text(iy*ps1,ix*ps0,"%.2e"%variance)
        ax.set_aspect('equal')
        #ax.axis('off')
        ax.set_title(field_names[ivar])
        divider = make_axes_locatable(ax)
        cax = divider.append_axes('right', size='5%', pad=0.05)
        plt.colorbar(cs, cax=cax, orientation='vertical')
    fig.tight_layout()
    plt.savefig(f"./imgs/check_all_{time_idx}.png")
    plt.close()
    sys.exit(-1)

def plot_refinedtokens(patch_size, field_names, time_idx, data, II_topk):
    T, nvar, H, W = data.shape
    ps = patch_size[-1]
    ps0=ps[0]
    ps1=ps[1]
    ntokenx = H//ps0
    ntokeny = W//ps1
    ncol=math.ceil(math.sqrt(nvar))
    nrow=nvar//ncol
    fig, axs = plt.subplots(nrow,ncol, figsize=(20, 20))

    for ivar in range(nvar):
        irow=ivar//ncol
        icol=ivar%ncol
        ax = axs[irow, icol]
        cs = ax.contourf(np.asarray(data[0, ivar, :240, :]).squeeze(), cmap="jet", levels=50)
        for iref in range(len(II_topk)):
            itoken = II_topk[iref]
            if itoken<0:
                continue
            ix=itoken.cpu()//ntokeny
            iy=itoken.cpu()%ntokeny
            assert ix*ntokeny+iy==itoken
            ax.plot([iy*ps1,     (iy+1)*ps1], [ix*ps0,         ix*ps0], "m--")
            ax.plot([iy*ps1,     (iy+1)*ps1], [(ix+1)*ps0, (ix+1)*ps0], "m--")
            ax.plot([iy*ps1,         iy*ps1], [ix*ps0,     (ix+1)*ps0], "m--")
            ax.plot([(iy+1)*ps1, (iy+1)*ps1], [ix*ps0,     (ix+1)*ps0], "m--")
            variance=data[:, ivar, ix*ps0:(ix+1)*ps0, iy*ps1:(iy+1)*ps1].var(axis=(0,-2,-1)).mean()
            ax.text(iy*ps1,ix*ps0,"%.2e"%variance)
        ax.set_aspect('equal')
        ax.axis('off')
        ax.set_title(field_names[ivar])
        divider = make_axes_locatable(ax)
        cax = divider.append_axes('right', size='5%', pad=0.05)
        plt.colorbar(cs, cax=cax, orientation='vertical')
        #ax.set_title(variance[II_topk[iref]].item())
    fig.tight_layout()
    plt.savefig(f"./imgs/check_ref_{time_idx}.png")
    plt.close()

def figure_checking(x_0, xlocal, B, ntokenx, ntokeny, ps0, ps1, refineind):

    x_0 = rearrange(x_0, '(b ntx nty) t c h w -> t b c (ntx h) (nty w)', ntx=ntokenx, nty=ntokeny)
    varmin = x_0.amin(dim=(0,1,-2,-1))
    varmax = x_0.amax(dim=(0,1,-2,-1))
    print(varmin, varmax)
    for ib in range(B):
        fig, axs = plt.subplots(2,2, figsize=(10, 10))
        for ivar in range(4):
            irow=ivar//2
            icol=ivar%2
            ax = axs[irow, icol]
            cs = ax.contourf(x_0[0, ib, ivar, :, :].squeeze().cpu().detach().numpy().transpose(), cmap="jet", levels=50)
            for iref in range(len(refineind)//B):
                itoken = refineind[iref+ib*ntokenx*ntokeny]
                if itoken<0:
                    continue
                ix=itoken.cpu()//ntokeny
                iy=itoken.cpu()%ntokeny
                assert ix*ntokeny+iy==itoken
                ax.plot([ix*ps0,         ix*ps0], [iy*ps1,     (iy+1)*ps1], "m--")
                ax.plot([(ix+1)*ps0, (ix+1)*ps0], [iy*ps1,     (iy+1)*ps1], "m--")
                ax.plot([ix*ps0,     (ix+1)*ps0], [iy*ps1,         iy*ps1], "m--")
                ax.plot([ix*ps0,     (ix+1)*ps0], [(iy+1)*ps1, (iy+1)*ps1], "m--")
            ax.set_aspect('equal')
            ax.axis('off')
        fig.tight_layout()
        plt.savefig(f"check_{ib}.png")
        plt.close()

    ibt = -1
    print(xlocal.size())
    for ib in range(B):
        fig, axs = plt.subplots(2,2, figsize=(10, 10))
        for iref in range(len(refineind)//B):
            itoken = refineind[iref+ib*ntokenx*ntokeny]
            if itoken<0:
                continue
            ix=itoken.cpu()//ntokeny
            iy=itoken.cpu()%ntokeny
            assert ix*ntokeny+iy==itoken
            ibt += 1
            for ivar in range(4):
                irow=ivar//2
                icol=ivar%2
                ax = axs[irow, icol]
                cs = ax.contourf(np.arange(ps0)+ps0*ix.item(), np.arange(ps1)+ps1*iy.item(),xlocal[ibt, 0, ivar, :, :].squeeze().cpu().detach().numpy().transpose(),
                                    vmin=varmin[ivar], vmax=varmax[ivar], cmap="jet", levels=50)
                ax.set_xlim(0, 256)
                ax.set_ylim(0, 256)
                ax.set_aspect('equal')
                ax.axis('off')
        fig.tight_layout()
        plt.savefig(f"check_local{ib}.png")
        plt.close()
    sys.exit(-1)
