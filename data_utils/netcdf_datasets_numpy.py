...
class BasenetCDFDirectoryDataset(Dataset):
    ...
    def __init__(self, path, include_string='', n_steps=1, dt=1, leadtime_max=1, split='train', 
                 train_val_test=None, subname=None, patch_size=None, refine_ratio=None, gammaref=None):
        super().__init__()
        self.path = path
        self.split = split
        if subname is None:
    ...
    def _calculate_patch_variance(self, data):
    ...
    def _get_directory_stats(self, path):
        self.files_paths = glob.glob(path + "/*.npy")
    ...
    def _loaddata_file(self, file_ind):
        self.datasets[file_ind] = np.load(self.files_paths[file_ind])
...
class  CollisionDataset(BasenetCDFDirectoryDataset):
    ...
    def _calculate_patch_variance(self, data, time_idx=None):
        ...
        #plot_refinedtokens(self.patch_size, self.field_names, time_idx, data, II_topk)
    ...
    def _reconstruct_sample(self, dat, leadtime, time_idx, n_steps):
        ...
        comb_x=dat[time_idx-n_steps:time_idx,:,:,:]

        t = time_idx-1+leadtime
        comb_y = dat[t:t+1] # 1, nz, nx, nc
