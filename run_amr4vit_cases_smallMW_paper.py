...
base_dir = "AMR4ViT_SmallMW_posbias_lr1e-4"
setlr=True
...
config_count = 0
#for ms in model_size[2:]:
for ms in model_size:
    for modelatt in model_attention: #[1:2]:
        pre_dir = os.path.join(base_dir, "%s_%s"%(ms, modelatt))
        os.makedirs(pre_dir, exist_ok=True)
        for adap in adaptivity[1:]:
            adapstr="adap" if adap else "baseline"
            #model_dir = os.path.join(pre_dir, adapstr+"_nowwarmup")
            ...
            for line in template_lines:
                if "for i in $(seq 1 1);" in line:
                    bash_script_content.append("for i in $(seq 1 1);\n")
                else:
